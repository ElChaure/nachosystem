...
/**
 * This is the model class for table "salidadet_tmp".
 *
 * The followings are the available columns in table 'salidadet_tmp':
 ...
 */
class Salidadet_tmp extends CActiveRecord
{
	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return Salidadet_tmp the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			array('salidaenc_id, producto_id, unidad_id, cantidad, precio_unitario, iva, total_neto, total_final', 'required'),
			array('salidaenc_id, producto_id, unidad_id, cantidad', 'numerical', 'integerOnly'=>true),
...
public function getTotiva()
        {
                ...
                $command=$connection->createCommand("SELECT SUM(total_final)-SUM(total_neto) as totiva FROM `salidadet_tmp`");
...
public function getTotneto()
        {
                ...
                $connection=Yii::app()->db;
                $command=$connection->createCommand("SELECT SUM(total_neto) as totneto FROM `salidadet_tmp`");
...
public function getTotfinal()
        {
                ...
                $connection=Yii::app()->db;
				$command=$connection->createCommand("SELECT SUM(total_neto) as totneto FROM `salidadet_tmp`");
				$totalnet= $command->queryScalar();
				Yii::app()->getSession()->add('totalnet', $totalnet);
                $command=$connection->createCommand("SELECT SUM(total_final) as totfinal FROM `salidadet_tmp`");
